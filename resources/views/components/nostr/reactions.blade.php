<div class="relative py-4 pl-16 ml-2" x-data="getReactions(event)">
    <template x-if="reactions[event.id]">
        <div class="flex justify-between w-full">
            <div class="cursor-pointer flex space-x-2" @click="love(event)">
                <div class="text-amber-500" x-text="reactions[event.id].reactions"></div>
                <x-fat-heart class="hover:text-amber-500"
                             ::class="reactions[event.id].reacted ? 'text-amber-500' : ''"/>
            </div>
            <div class="cursor-pointer flex space-x-2" @click="zap(event)">
                <div class="text-amber-500" x-text="reactions[event.id].zaps"></div>
                <x-fat-bolt class="hover:text-amber-500"/>
            </div>
            <div class="cursor-pointer flex space-x-2" @click="repost(event)">
                <div class="text-amber-500" x-text="reactions[event.id].reposts"></div>
